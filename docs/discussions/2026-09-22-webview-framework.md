...
Further local contract evidence: [activityProjection.ts](../../src/adapter/runtime/activityProjection.ts) correlates activity by assistant message ID, not by user turn. The session catalogue supplies title/excerpt/modified metadata, sorted newest first and paged; a recent list can use that existing projection. [toolApproval.ts](../../src/extension/editor-tools/toolApproval.ts) permits up to eight pending approvals, each with its own identity and expiry. Frontend navigation and selective expansion must not renew expiry, batch-authorize requests or silently switch the live session.
