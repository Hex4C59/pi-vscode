...
仅转发有界展示字段，不发送原始 RPC 对象／stderr、签名或凭证存储。模式过滤尽力而为，不证明任意 thinking／工具输出不含敏感信息。不宣称沙箱。`scripts/spike-approval.mjs` 针对已安装 `0.86.1` 的离线证据覆盖两个写调用、允许／拒绝、超时／迟到、Stop 与加载失败，使用隔离 fixture provider，不使用真实 provider 凭证。这不是 F5／真实 provider 验收。已报告 73 tests、compile／lint 与九个审批夹具通过，含运行中 PowerShell Stop 和第三方发现／settings／package 排除 marker。UI 的一次／会话／拒绝、精确授权查看／撤销、持续 stopping 的 Stop、草稿保留与下一轮控件已实现并测试。生产强制 `PI_OFFLINE=1`／`PI_TELEMETRY=0`，保留可信用户 provider 配置与凭证命令，不对其执行提供沙箱。`scripts/spike-offline-inference.mjs` 使用 `--offline` 验证跳过缺包启动安装且实际 loopback HTTP 推理成功；单元测试另覆盖继承环境覆盖。完整依赖 `dist/pi-vscode-validation.vsix`（139.71 MB／14,080 entries）通过 `scripts/verify-vsix.mjs` 解压与独立固定 CLI／gate hello/get_state 验证（无工具探针）。以上为此前报告的检查，不是本次文档收尾重跑。2026-09-21 维护者确认开发态 F5 的 thinking／状态、文件读取／工具卡、拒绝写入无副作用后允许写入、长时间无害命令 Stop，随后要求 WI-010 收尾。这不确认已安装 VSIX 激活／运行、完整授权复用／撤销／重置或剩余手动审批／生命周期／UI 矩阵。WI-008／WI-009 延期 F5（含审批／Stop 顺序）、pending ADR 与全部开放 gate 仍待完成；见[收尾证据](../archive/2026-09-21-closed-wi-history.zh.md#wi-010) 与 ACTIVE。
