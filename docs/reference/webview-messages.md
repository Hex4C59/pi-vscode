...
> Connectivity and pre-runtime workspace choice only. No chat, secrets, pi SDK, filesystem access or generic host operations are exposed. Both trust gates remain open.
...
- Host → webview `workspaceState`: `{ version: 1, type: "workspaceState", generation, status, folder, choice, busy, error, runtime: "not-started" }`. Generation is a nonnegative safe integer. Status is `no-folder`, `multi-root`, `remote`, `non-file`, `untrusted` or `eligible`. Folder is null or `{ name, path }` for a single folder. Choice is null, `allow` or `decline`; busy is boolean; error is null or a fixed, bounded user-facing retry message (never raw exception text).
...
- Allow/decline is editable and memory-only. Allow warns of future project extension/package execution; decline still permits AGENTS.md context and user/global resources in future integration. Neither is a sandbox or tool authorization. Every state says runtime not started; no pi runtime is imported or called.
